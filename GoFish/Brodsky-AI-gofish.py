...
def determine_num_players(): #determines how many human players are in the game
	n = int(input("How many players? (1-7): "))
	if(n >= 1 and n <= 7):
		player_num = n
	else:
		print("The number of players must be between 1 and 7")
		player_num, hand_card_num = determine_num_players()
	return player_num, 7 #default is 7

def determine_ai_players(human_num):
	if(human_num > 1):
		return 0
	else:
		m = input("How many A.I. players? (1-6): ")
		if(m.isnumeric()):
			n = int(m)
			if(n <= 6 and n >= 1):
				x = n
				...
				x = determine_ai_players(human_num)
		else:
...
def deal_hand(deck, num_cards, players):
	...
	for x in range(num_cards * players): 
		...
		deck.pop(0)
	return hands, deck, banned_cards
...
def main():
	...
	gameRunning = True
	human_num, hand_card_num = determine_num_players()
	ai_num = determine_ai_players(human_num)
	players = human_num + ai_num
	new_deck = create_deck()
	shuffle_deck(new_deck)
	hand_decks, deck_pile, banned_cards = deal_hand(new_deck, hand_card_num, players)
	...
	while(gameRunning):
		for x in range(players):
			...
			if(len(hand_decks[current_player]) > 0 or len(deck_pile) > 0): #if a user has no more cards in their hand or if the deck has no cards
				...
				os.system('clear')
				if((ai_num > 0 and current_player == 1) or (ai_num == 0)):
				...
				hand_decks, deck_pile = handle_guess(current_player, hand_decks, deck_pile, players, book_of_tricks, banned_cards) #human + AI players make a guess
